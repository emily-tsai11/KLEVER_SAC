				...
				if(sacHC)
				{
					...
					if(n_hit > 0)
					{
						...
						for(G4int i = 0; i < n_hit; i++)
						{
							...
							hit->SetPosition(TVector3((*sacHC)[i]->GetPosX(), (*sacHC)[i]->GetPosY(), (*sacHC)[i]->GetPosZ()));
				...
				if(sacDC)
				{
					...
					if(n_digi > 0)
					{
						...
						for(G4int i = 0; i < n_digi; i++)
						{
							...
							for(G4int j = 0; j < digi->GetTHistoNBins(); j++)
								digi->SetTHistoBin(j, (*sacDC)[i]->GetQHistoBin(j));
