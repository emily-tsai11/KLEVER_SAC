...
SACMessenger::SACMessenger(SACDetector* det) : fSACDetector(det)
{
	...
	fSetCrystalGapCmd->SetRange("G > 0.0 && G <= 1000.0");
...
void SACMessenger::SetNewValue(G4UIcommand* cmd, G4String par)
{
	...
	if(cmd == fSetCrystalGapCmd) fSACGeometry->SetCrystalGap(fSetCrystalGapCmd->GetNewDoubleValue(par));
