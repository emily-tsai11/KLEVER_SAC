...
SACDigi::SACDigi()
{
	...
	fQHistoStart = 0.0 * ns; // assume that ADC sampling starts at bunch start time
...
SACDigi::SACDigi(const SACDigi& right) : G4VDigi()
{
...
void SACDigi::Print()
{
	G4cout << "SAC - channel: " << fChannelId << " time: " << G4BestUnit(fTime, "Time") << " energy: " << G4BestUnit(fEnergy, "Energy") << G4endl;
	// PrintQHisto();
}
...
{
	for(G4int i = 0; i < SACDIGI_N_BINS; i++) fQHisto[i] = 0.0;
}
...
void SACDigi::PrintQHisto()
{
	...
	for(G4int i = 0; i < SACDIGI_N_BINS; i++) printf("%3.3d %9.3f\n", i, fQHisto[i]);
}
