...
{
	G4String HCname;
	collectionName.insert(HCname = "PMTCollection"); // create the collection name
}
...
{
	fPMTCollection = new PMTHitsCollection(SensitiveDetectorName, collectionName[0]);
	static G4int HCID = -1;
	if(HCID < 0) HCID = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[0]);
	HCE->AddHitsCollection(HCID, fPMTCollection);
}
...
// G4int PMTSD::ClassifyTrack(G4Track* track)
// {
...
void PMTSD::EndOfEvent(G4HCofThisEvent*)
{
	// if(verboseLevel > 0)
	// {
	// 	G4int NbHits = fPMTCollection->entries();
	// 	G4cout << "\n-- PMT Hits Collection: " << NbHits << " hits --" << G4endl;
	// 	for(G4int i = 0; i < NbHits; i++) (*fPMTCollection)[i]->Print();
	// }
	//
	// G4AnalysisManager* fAnalysisManager = G4AnalysisManager::Instance();
	//
	// G4int nHitEntries = fPMTCollection->entries();
	// std::map<G4int, G4bool> trackedHits;
	//
	// PMTGeometry* geo = PMTGeometry::GetInstance();
	...
	// {
	// 	PMTHit* currentHit = (*fPMTCollection)[i];
	//
	...
	//
	// 		// fill number of particle hits in PMT by layer
	// 		fAnalysisManager->FillH1(partType + 8 * nParticles, z, 1.0);
	//
	// 		// fill x-y plane of PMT hits
	...
	// 		NPerEvent[partType]++;
	//
	// 		// TEMPORARY, UNTIL BACK OF PMT IS MADE NON-REFLECTIVE
	// 		if(partType == 10)
